function test_gradient_whole_net()
    addpath(genpath('../'));
    % test the gradient using
    % the evaluate funtion
    ...
    net.layers{end+1} = struct('type', 'fully_connected',...
        'params', struct('weights', randn(5,6), 'biases', randn(5,1)));
    
    net.layers{end+1} = struct('type', 'relu');
    
    net.layers{end+1} = struct('type', 'fully_connected',...
        'params', struct('weights', randn(3,5), 'biases', randn(3,1)));
    ...
    for k=1:3
        ...
        test_gradients(@(y) change_one_param(y, net, x, labels, 4, 'biases'), grads{4}.biases, net.layers{4}.params.biases, 1e-5, 5);
        test_gradients(@(y) change_one_param(y, net, x, labels, 4, 'weights'), grads{4}.weights, net.layers{4}.params.weights, 1e-5, 30);
