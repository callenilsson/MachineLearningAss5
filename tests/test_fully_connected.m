function test_fully_connected()
    ...
    res = zeros(2,1);
    res(:,1) = [5 1];
    test_equal(fully_connected_forward(x, A, b), res, 1e-5, 'Forward pass with batch of one is incorrect');

    b(:,1) = [2 3];
    res(:,1) = [7 4];
    test_equal(fully_connected_forward(x, A, b), res, ...
        1e-5, 'Forward pass with batch of one is incorrect. Check the bias');
    ...
    test_equal(fully_connected_forward(x, A, b), res, ...
        1e-5, 'Forward pass with batch of two is incorrect.');
    ...
    res = [17 41];
    test_equal(fully_connected_forward(x, A, b), res, ...
        1e-5, 'Forward pass with input that is not vectorized fail. Make sure that you reshape x to size [features, elements in the batch].');
